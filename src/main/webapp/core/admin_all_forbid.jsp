...
<script>
	function searchannel(){
		...
		window.location.href = "admin_all_view.do?real_name=" + name;	
	}
	...
	function topage(){
		...
		if (page <= pageCount){
			window.location.href = "admin_all_view.do?real_name=${name}&id=" + page;	
		} else {
  			...
  			data: 'forbid=0&id=' + id,
   			...
   			success: function(data){
       			if(data.hint == "success"){
       				$(location).attr("href", "admin_all_view.do?real_name=${name}&id=${thisPage}");
...
<div class="navcon">
    <div class="con_title">
        <a href="#">首页</a><span>/</span><a href="#">渠道管理</a><span>/</span><a href="#">禁用列表</a>
        <div class="ssbox">
    ...
    <div class="pagesite">
        ...
        <tr>
  			<c:if test="${thisPage > 1}">
  					<td><a href="admin_all_view.do?id=${thisPage-1}&real_name=${name}">上一页</a></td>
			</c:if>
			
			<c:if test="${thisPage > 3}">
				<c:forEach begin="1" end="3" var="i" step="1">
					<td><a href="admin_all_view.do?id=${i}&real_name=${name}">${i}</a></td>
				</c:forEach>
				<c:if test="${thisPage > 6}">
        			<td class="shenglue"><a href="#">...</a></td>
        		</c:if>
        		<c:if test="${thisPage == 5}">
        			<td><a href="admin_all_view.do?id=4&real_name=${name}">4</a></td>
        		</c:if>
        		<c:if test="${thisPage >= 6}">
        			<td><a href="admin_all_view.do?id=${thisPage-2}&real_name=${name}">${thisPage-2}</a></td>
        			<td><a href="admin_all_view.do?id=${thisPage-1}&real_name=${name}">${thisPage-1}</a></td>
        		</c:if>
			</c:if>
			...
			<c:if test="${thisPage + 3 == pageCount}">
				<td><a href="admin_all_view.do?id=${pageCount}&real_name=${name}">${pageCount}</a></td>
			</c:if>
			...
			<c:if test="${thisPage + 3 < pageCount}">
				...
				<td><a href="admin_all_view.do?id=${pageCount-1}&real_name=${name}">${pageCount-1}</a></td>
				<td><a href="admin_all_view.do?id=${pageCount}&real_name=${name}">${pageCount}</a></td>
			</c:if>
 			...
 			<c:if test="${thisPage+1 <= pageCount}">
  					<td><a href="admin_all_view.do?id=${thisPage+1}&real_name=${name}">下一页</a></td>
			</c:if>
         </tr>
            </table>
