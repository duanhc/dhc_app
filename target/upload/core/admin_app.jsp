...
<script>
    function searchannel() {
        ...
        window.location.href = "admin_app_view.do?real_name=" + name;
    }
    ...
    function topage() {
        ...
        if (page <= pageCount) {
            window.location.href = "admin_app_view.do?real_name=${name}&id=" + page;
        } else {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.hint == "success") {
                    $(location).attr("href", "admin_app_view.do?real_name=${name}&id=${thisPage}");
...
<div class="navcon">
    <div class="con_title">
        <a href="#">首页</a><span>/</span><a href="#">管理员管理</a><span>/</span><a href="#">产品管理员</a>
        <div class="ssbox">
            <input type="text" placeholder="请输入真实姓名" id="searchname" value="${name}" class="fl" style="height:31px; outline:none;"/>
    ...
    <div class="pagesite">
        ...
        <tr>
  			<c:if test="${thisPage > 1}">
                <td><a href="admin_app_view.do?id=${thisPage-1}&real_name=${name}">上一页</a></td>
            </c:if>
			
			<c:if test="${thisPage > 3}">
                <c:forEach begin="1" end="3" var="i" step="1">
                    <td><a href="admin_app_view.do?id=${i}&real_name=${name}">${i}</a></td>
                </c:forEach>
                <c:if test="${thisPage > 6}">
                    <td class="shenglue"><a href="#">...</a></td>
                </c:if>
                <c:if test="${thisPage == 5}">
                    <td><a href="admin_app_view.do?id=4&real_name=${name}">4</a></td>
                </c:if>
                <c:if test="${thisPage >= 6}">
                    <td><a href="admin_app_view.do?id=${thisPage-2}&real_name=${name}">${thisPage-2}</a></td>
                    <td><a href="admin_app_view.do?id=${thisPage-1}&real_name=${name}">${thisPage-1}</a></td>
                </c:if>
            </c:if>
			...
			<c:if test="${thisPage + 3 == pageCount}">
                <td><a href="admin_app_view.do?id=${pageCount}&real_name=${name}">${pageCount}</a></td>
            </c:if>
			...
			<c:if test="${thisPage + 3 < pageCount}">
                ...
                <td><a href="admin_app_view.do?id=${pageCount-1}&real_name=${name}">${pageCount-1}</a></td>
                <td><a href="admin_app_view.do?id=${pageCount}&real_name=${name}">${pageCount}</a></td>
            </c:if>
 			...
 			<c:if test="${thisPage+1 <= pageCount}">
                <td><a href="admin_app_view.do?id=${thisPage+1}&real_name=${name}">下一页</a></td>
            </c:if>
         </tr>
            </table>
