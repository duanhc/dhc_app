...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.hint == "success") {
                    $(location).attr("href", "app_put_view.do?id=${thisPage}&name=${name}&put_away=${put}");
    ...
    function searchapp() {
        ...
        window.location.href = "app_put_view.do?name=" + name;
    }
    ...
    function topage() {
        ...
        if (page <= pageCount) {
            window.location.href = "app_put_view.do?name=${name}&put_away=${put}&id=" + page;
        } else {
...
<div class="navcon">
    <div class="con_title">
        <a href="#">首页</a><span>/</span><a href="#">产品管理</a><span>/</span><a href="#">上架管理</a>
        <div class="ssbox">
            <input type="text" placeholder="请输入产品名称" value="${name}" id="searchname" class="fl" style="height:31px; outline:none;"/>
            <a href="#" onClick="searchapp()">
                <div class="fl" style="background-color:#009786;width:60px; padding:0 0 0 15px;height:35px; line-height:35px; font-size:13px; color:#fff;"><img src="core/images/ss.png"
            ...
            <c:forEach var="item" items="${apps}">
                <tr>
                    ...
                    <td>
                        ...
                        <a class="upframe" onclick="delApp(${item.id})">删除</a>
                    </td>
                </tr>
            </c:forEach>
    ...
    <div class="pagesite">
        ...
        <tr>
  			<c:if test="${thisPage > 1}">
                <td><a href="app_put_view.do?id=${thisPage-1}&name=${name}&put_away=${put}">上一页</a></td>
            </c:if>
			
			<c:if test="${thisPage > 3}">
                <c:forEach begin="1" end="3" var="i" step="1">
                    <td><a href="app_put_view.do?id=${i}&name=${name}&put_away=${put}">${i}</a></td>
                </c:forEach>
                <c:if test="${thisPage > 6}">
                    <td class="shenglue"><a href="#">...</a></td>
                </c:if>
                <c:if test="${thisPage == 5}">
                    <td><a href="app_put_view.do?id=4&name=${name}&put_away=${put}">4</a></td>
                </c:if>
                <c:if test="${thisPage >= 6}">
                    <td><a href="app_put_view.do?id=${thisPage-2}&name=${name}&put_away=${put}">${thisPage-2}</a></td>
                    <td><a href="app_put_view.do?id=${thisPage-1}&name=${name}&put_away=${put}">${thisPage-1}</a></td>
                </c:if>
            </c:if>
			...
			<c:if test="${thisPage + 3 == pageCount}">
                <td><a href="app_put_view.do?id=${pageCount}&name=${name}&put_away=${put}">${pageCount}</a></td>
            </c:if>
			...
			<c:if test="${thisPage + 3 < pageCount}">
                ...
                <td><a href="app_put_view.do?id=${pageCount-1}&name=${name}&put_away=${put}">${pageCount-1}</a></td>
                <td><a href="app_put_view.do?id=${pageCount}&name=${name}&put_away=${put}">${pageCount}</a></td>
            </c:if>
 			...
 			<c:if test="${thisPage+1 <= pageCount}">
                <td><a href="app_put_view.do?id=${thisPage+1}&name=${name}&put_away=${put}">下一页</a></td>
            </c:if>
         </tr>
            </table>
